...
VAULT_SA_SECRET_NAME=$(wget -vO- -nv --ca-certificate /var/run/secrets/kubernetes.io/serviceaccount/ca.crt --header "Authorization: Bearer $KUBE_TOKEN" https://kubernetes.default/api/v1/namespaces/$namespace/serviceaccount/vault | jq -r -e '.secrets[] | select(.name | test("vault-token")) | .name');
...
vault kv put k8s/secrets/vault root_token=$(cat cluster-keys.json | jq -r '.root_token');
